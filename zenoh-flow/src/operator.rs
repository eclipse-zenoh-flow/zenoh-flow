...
use crate::types::{Token, ZFResult, ZFContext, ZFLinkId};
...
pub type FnInputRule = dyn Fn(&mut ZFContext, &mut HashMap<ZFLinkId, Token<dyn DataTrait>>) -> InputRuleResult + Send + Sync + 'static;


pub type OutputRuleResult = ZFResult<HashMap<ZFLinkId, Arc<ZFMessage>>>;

pub type FnOutputRule = dyn Fn(&mut ZFContext, HashMap<ZFLinkId, Arc<dyn DataTrait>>) -> OutputRuleResult + Send + Sync + 'static;


pub type RunResult = ZFResult<HashMap<ZFLinkId, Arc<dyn DataTrait>>>;

pub type FnRun = dyn Fn(&mut ZFContext, HashMap<ZFLinkId, Arc<dyn DataTrait>>) -> RunResult + Send + Sync + 'static;
