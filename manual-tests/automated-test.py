...
if (
    os.path.exists(os.path.join(manual_tests_path, "test.json")) is False
    or os.path.exists(os.path.join(manual_tests_path, "zenoh-zf-plugin.json")) is False
    or zf_command["zenohd_cmd"] is None
    or zf_command["runtimes_cmd"] is None
    or zf_command["dataflow_yaml_cmd"] is None
    or zf_command["instances_cmd"] is None
):
    os.chdir(zenoh_path)
    ...
    zf_command["dataflow_yaml_cmd"] = (
        zf_conf.zfctl_path + " launch " + getting_started_path[0]
    )
...
with open(getting_started_path[0], "r") as f:
    ...
    if str(tuple(data["sources"]).__len__()) == instance["sources"]:
        print(
            "[info] test active Source nodes: SUCCESS, the active Source nodes [",
            str(tuple(data["sources"]).__len__()),
            "] are equal to declared Source nodes [",
            instance["sources"],
            "]",
        )
    if str(tuple(data["sinks"]).__len__()) == instance["sinks"]:
        print(
            "[info] test active Sink nodes: SUCCESS, the active Sink nodes [",
            str(tuple(data["sinks"]).__len__()),
            "] are equal to those declared Sinks nodes [",
            instance["sinks"],
            "]",
        )
    if str(tuple(data["operators"]).__len__()) == instance["operators"]:
        print(
            "[info] test active Operator nodes: SUCCESS, the active Operator nodes [",
            str(tuple(data["operators"]).__len__()),
            "] are equal declared Operator nodes [",
            instance["operators"],
            "]",
        )
    if (
