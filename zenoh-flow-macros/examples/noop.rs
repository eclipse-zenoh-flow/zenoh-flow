...
// #[zfoperator(
//     modes = {"default": {"ir" : 'NoOp::ir_1, "run": NoOp::run_1, "or" :NoOp::or_1}},
...
impl NoOp {
    ...
    fn ir_1(&self, data : &(Option<u128>, Option<String>)) -> (bool, (InputRuleResult, InputRuleResult)) {
        match data {
...
impl NoOp {
    ...
    fn deserialize_state(data : &[u8]) -> NoOp {
        zenoh_flow::bincode::deserialize::<NoOp>(data).unwrap()
    }

    fn serialize_state(data : NoOp) -> Vec<u8> {
        zenoh_flow::bincode::serialize(&data).unwrap()
    }

    // KPN-like default Input Rule
    fn default_ir(&self, data : &(Option<u128>, Option<String>)) -> (bool, (InputRuleResult, InputRuleResult)) {
        match data {
            (Some(_),Some(_)) => (true, (InputRuleResult::Process, InputRuleResult::Process)),
            (None, Some(_)) => (false, (InputRuleResult::Wait, InputRuleResult::Process)),
            (Some(_), None) => (false, (InputRuleResult::Process, InputRuleResult::Wait)),
    ...
    fn default_or(&mut self, data : (Option<String>, Option<u128>)) -> (Option<String>, Option<u128>) {
        data
    }
}

// Generated by Macro
impl zenoh_flow::ZFOperator for NoOp {
    ...
    fn make_run(&self, ctx: &mut zenoh_flow::ZFContext) -> Box<zenoh_flow::OperatorRun> {
        ...
        match ctx.mode.as_str() {
            "default" => {
                 Box::new(
                    | ctx: &mut zenoh_flow::ZFContext, data: Vec<Option<&zenoh_flow::message::Message>> | -> zenoh_flow::OperatorResult {

                    // Zenoh Flow Ctrl IR - Inner
                    let inputs = (Some(NoOp::deserialize_input_0(data[0].unwrap().data())), Some(NoOp::deserialize_input_1(data[1].unwrap().data())));
                    let mut state = NoOp::deserialize_state(&ctx.state);
                    //

                    // User IR
                    match NoOp::ir_1(&state, &inputs) {
                        (false, (x,y)) => {
                            // We should recover information in case of InputRuleResult::Drop, because we need to drop data
                            return zenoh_flow::OperatorResult::InResult(Ok((false, vec![x,y])))
                        }
                        _ => {


                            // User Run
                            let run_res = NoOp::run_1(&mut state, inputs);

                            //User OR
                            let results = NoOp::or_1(&mut state, run_res);

                            // Zenoh Flow Ctrl OR - Inner
                            let outputs = vec![
                                zenoh_flow::message::Message::new(NoOp::serialize_output_0(results.0.unwrap())),
                                zenoh_flow::message::Message::new(NoOp::serialize_output_1(results.1.unwrap()))
                                ];
                            ctx.state = NoOp::serialize_state(state);
